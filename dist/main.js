...
const photographyDropdownToggle = document.getElementById('photographyDropdownToggle');
const photographyDropdownContent = document.getElementById('photographyDropdownContent');
...
window.addEventListener('click', (event) => {
    if (photographyDropdownContent && photographyDropdownToggle) {
        ...
        if (event.target !== photographyDropdownToggle && !photographyDropdownContent.contains(event.target)) {
...
function populateGrid(images, callback) {
    ...
    images.forEach(image => {
        ...
        imgElement.addEventListener('click', () => {
            openLightbox(imgElement.dataset.fullSrc || '', imgElement.dataset.alt || 'Enlarged view', imgElement.dataset.caption);
        });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (galleryDataElement && imageGridElement) {
        try {
            const imageData = JSON.parse(galleryDataElement.textContent || '[]');
            console.log("Parsed image data:", imageData);
            if (imageData.length > 0) {
                populateGrid(imageData, () => {
                    // Initialize Masonry after images are loaded
                    const masonry = new window.Masonry(imageGridElement, {
                        itemSelector: 'img',
